...
# Date: 2021-08-18
#
# Description: Add modified Graphconv;
# #################################
...
class GCN(nn.Module):
    def __init__(self, in_feats, n_hidden, n_classes, n_layers, activation, dropout, use_linear,
        ...
        for i in range(n_layers):
            ...
            self.convs.append(GraphConv(in_hidden, out_hidden, bias=bias))  ## (?>?<?)
            if use_linear:
        ...
        self.CL_dim = CL_dim
        if self.CL_dim is None:
            self.CL_linear = None
        else:
            self.CL_linear = nn.Linear(n_hidden, self.CL_dim)
